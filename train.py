...
dataset = DicomNii2DDataset(
    "./20241008_smart_health_care2_abnormal_public_001_200/breast", transform
)
...
with open("train_log.txt", "w") as log_file:
    for i in range(n_samples):
        ...
        train_loader = DataLoader(
            dataset, batch_size=BATCH_SIZE, sampler=train_sampler, num_workers=2
        )
        val_loader = DataLoader(
            dataset, batch_size=1, sampler=val_sampler, num_workers=2
        )
        ...
        train_losses, train_gds, train_miou = train(
            model, num_epochs, learning_rate, train_loader, device
        )
        ...
        print(
            f"[VAL] LOOCV Fold {i + 1}/{n_samples} | GDS: {avg_val_gds}, mIoU: {avg_val_miou}, score(GDS + mIoU): {avg_val_score}"
        )
        log_file.write(
            f"[VAL] LOOCV Fold {i + 1}/{n_samples} | GDS: {avg_val_gds}, mIoU: {avg_val_miou}, score(GDS + mIoU): {avg_val_score}\n"
        )
        ...
        if avg_val_score > best_score:
            ...
            plt.plot(
                epochs, [gds.cpu().numpy() for gds in train_gds], label="Train GDS"
            )
            plt.plot(
                epochs, [miou.cpu().numpy() for miou in train_miou], label="Train mIoU"
            )
